...
import { DocumentBreadCrumbs, DocumentPagesBreadCrumbs, LoadingBreadCrumbs } from "@helpers/ui";
    ...
    setBreadCrumbs$: events.on(UpdateDocumentsAction, SetViewingDocumentIdAction).pipe(
      map(() => {
        ...
        if (store.viewingDocumentId() > 0) {
          ...
          const breadCrumb: BreadCrumbs = viewDocument
            ? { title: viewDocument.name }
            : LoadingBreadCrumbs;

          breadCrumbs.push(DocumentPagesBreadCrumbs, breadCrumb);
        } else {
          const { link, ...breadCrumb } = DocumentPagesBreadCrumbs;
